...
def draw_marker(frame, action, width, height):
    if action["type"] == "click":
        ...
        cv2.circle(
            frame, (x, y), 20, (0, 255, 255), -1
        )  # Bigger circle with bright yellow color
        cv2.putText(
            frame,
            "Click",
            (x + 25, y + 10),
        ...
        cv2.line(
            frame, (start_x, start_y), (end_x, end_y), (255, 255, 0), 4
        )  # Thicker line with cyan color
        cv2.putText(
            frame,
            "Drag",
            (end_x + 25, end_y + 10),
...
def mark_video(log_file, video_file, output_file, time_offset):
    ...
    while cap.isOpened():
        ...
        while (
            ...
            and actions[action_index][
                "time" if "time" in actions[action_index] else "start_time"
            ]
            + time_offset
            <= current_frame / fps
        ):
...
def main(selected_folder, time_offset=0):
    ...
    for log_filename in os.listdir(log_folder):
        if log_filename.endswith(".json"):
            ...
            video_filename = log_filename.replace("log_", "screen_").replace(
                ".json", ".mp4"
            )
            video_file = os.path.join(video_folder, video_filename)
            output_file = os.path.join(
                output_folder, video_filename.replace(".mp4", "_marked.mp4")
            )
